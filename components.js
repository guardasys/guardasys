...
// ============================================================================
// Cada punto de guarda tiene su PROPIO mueble "matriz" — cantidad de filas
// y columnas configurable por punto (Administración → Puntos de guarda),
// porque los muebles físicos no son todos del mismo tamaño. Si un punto
// todavía no tiene filas/columnas cargadas (puntos creados antes de esta
// función), se usa el tamaño por defecto 4x10 — el mismo que tenían todos
// hardcodeado hasta ahora, así no se rompe nada hasta que se editen.
//
...
// 5, o si ya no queda ningún box libre en la matriz de ESE punto, esos
// volúmenes van directo al mueble "F" (un solo espacio, sin subdivisiones,
// igual para todos los puntos de guarda) como desborde. Las maletas nunca
// entran en la matriz por tamaño: van directo a F siempre, sin importar
// cuántas traiga el cliente ni cuántos otros volúmenes tenga.
//
...
const FILAS_MATRIZ_DEFAULT = 4;
const COLUMNAS_MATRIZ_DEFAULT = 10;
const LIMITE_VOLUMENES_POR_BOX = 5; // por cliente/guarda, sin contar maletas

/** Letras de fila A, B, C... para la cantidad pedida (26 como máximo práctico). */
function generarLetrasFilas(cantidad) {
  const letras = [];
  for (let i = 0; i < cantidad; i++) letras.push(String.fromCharCode(65 + i));
  return letras;
}

/**
 * Recorre la matriz (del tamaño de ESE punto de guarda) en orden fijo y
 * devuelve el primer box que no está ocupado por ninguna otra guarda
 * abierta (`boxesOcupados` es un Set de claves "FilaColumna", ej. "B7").
 * null si la matriz está llena.
 */
function buscarBoxLibre(boxesOcupados, filas, columnas) {
  for (const fila of generarLetrasFilas(filas)) {
...
/**
 * Asigna la ubicación de TODOS los volúmenes de una guarda a la vez, para
 * la matriz del punto de guarda con `filas` x `columnas` boxes. Las
 * maletas van directo a F. El resto (hasta LIMITE_VOLUMENES_POR_BOX)
 ...
 */
function asignarUbicacionesOperacion(volumenes, boxesOcupados, filas, columnas) {
  const noMaletas = volumenes.filter((v) => v.tipo !== "maleta");
  const boxCompartido = noMaletas.length > 0 && noMaletas.length <= LIMITE_VOLUMENES_POR_BOX ? buscarBoxLibre(boxesOcupados, filas, columnas) : null;
...
function NuevaGuarda({ usuario }) {
  ...
  async function registrarGuarda() {
    ...
    const volumenesConUbicacion = asignarUbicacionesOperacion(
      volumenes,
      boxesOcupados,
      puntoGuarda.filas || FILAS_MATRIZ_DEFAULT,
      puntoGuarda.columnas || COLUMNAS_MATRIZ_DEFAULT
    );
